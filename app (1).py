...
# ── BNDE : format tableau Date | Valeur | Libellé | Débit | Crédit | Solde
# PDF natif texte — positions mesurées sur PDF BNDE Thiès (jan 2026) :
#   Date opération : x0 < 80   (DD/MM/YYYY)
#   Valeur         : x0 ≈ 80-140 (DD/MM/YYYY — ignoré)
#   Libellé        : x0 ≈ 140-370
#   Débit (XOF)    : x0 ≈ 370-460
#   Crédit (XOF)   : x0 ≈ 460-550
#   Solde (XOF)    : x0 ≥ 550  (ignoré)
# Particularité : montants XOF entiers sans décimales, fragmentés en plusieurs tokens
def parse_bnde(pages_words, pages_text, _pdf_path=''):
    ...
    SKIP = {'TOTAL', 'SOLDE', 'DATE', 'LIBELLÉ', 'LIBELLE', 'VALEUR', 'DÉBIT', 'DEBIT',
            'CRÉDIT', 'CREDIT', 'A REPORTER', 'SOLDE À REPORTER', 'SOLDE A REPORTER',
            'TITULAIRE', 'RELEVE', 'RELEVÉ', 'VEUILLEZ', 'PAGE', 'BNDE', 'AGENCE',
            'COMPTE', 'DEVISE', 'DOMICILIATION', 'SIÈGE', 'SOCIAL', 'RCCM', 'NINEA',
            'EXTRAIT', 'NOM DU CLIENT', 'SOLDE INITIAL', 'SOLDE (XOF)', 'DÉBIT (XOF)',
            'CRÉDIT (XOF)', 'SOLDE AU'}
    SKIP_START = ('SOLDE', 'TOTAL', 'A REPORTER', 'PAGE', 'BNDE', 'CODE')
    ...
    for pw in pages_words:
        ...
        while i < len(rows):
            ...
            date_words = [w for w in row if w['x0'] < 80
                          and re.match(r'^\d{2}/\d{2}/\d{4}$', w['text'])]
            if not date_words:
                i += 1; continue
            ...
            # Libellé : x0 ≈ 140-370
            label_words = [w for w in row if 140 <= w['x0'] < 370]
            label = ' '.join(w['text'] for w in label_words).strip()
            label_up = label.upper()
            ...
            # Montants débit/crédit (entiers XOF fragmentés)
            debit_words  = [w for w in row if 370 <= w['x0'] < 460]
            credit_words = [w for w in row if 460 <= w['x0'] < 550]
            debit_amt  = _uba_join_amount(debit_words)
            credit_amt = _uba_join_amount(credit_words)
            ...
            while j < len(rows):
                ...
                if r2 and any(w['x0'] < 80 and re.match(r'^\d{2}/\d{2}/\d{4}$', w['text'])
                               for w in r2):
                    break
                nl = ' '.join(w['text'] for w in r2 if 140 <= w['x0'] < 370).strip()
    ...
    # Fallback : parsing texte brut ligne par ligne avec mots-clés
    if not txns:
        full_lines = '\n'.join(pages_text).splitlines()
        date_re = re.compile(r'^(\d{2}/\d{2}/\d{4})\s+(\d{2}/\d{2}/\d{4})?\s*(.+)')
        credit_kw = {'VERSEMENT', 'VIREMENT', 'RECU', 'REMISE', 'CREDIT', 'DÉBLOCAGE',
                     'DEBLOCAGE', 'ANNUL', 'RECOUVREMENT', 'SWIFT', 'PAR :', 'PAR:',
                     'TIRE :', 'REMISE DE CHEQUES', 'SORT CHEQUES', 'TRF-RECU', 'TRF RECU'}
        debit_kw  = {'RETRAIT', 'CHEQ', 'CHQ', 'AGIOS', 'FRAIS', 'COMMISSION', 'APPEL',
                     'ABONNEMENT', 'ROUTAGE', 'VIREMENT AUTRE', 'FAV :', 'FAV:',
                     'BENEF :', 'BENEF:', 'BENEFICIAIRE', 'IBE', 'PACK'}
        ...
        for line in full_lines:
            ...
            # Extraire les montants entiers XOF (sans décimale)
            amounts = re.findall(r'\b(\d{1,3}(?:\s\d{3})+|\d{4,9})\b', rest)
            amounts_parsed = []
            for a in amounts:
                v = parse_amount(a.replace(' ', ''))
                if v is not None and v >= 100:
            ...
            # Libellé
            label_m = re.match(r'^([A-Za-zÀ-ÿ\s\-\'\./,&:°]+)', rest)
            ...
            is_credit = any(k in rest_up for k in credit_kw)
            is_debit  = any(k in rest_up for k in debit_kw)
            amt = amounts_parsed[0] if amounts_parsed else None
            if amt and amt > 0:
...
def parse_sg_afrique(pages_words, pages_text, _pdf_path=''):
    """
    Parser SGBS / Société Générale Sénégal.
    Format : Date | Valeur | Libellé | Débit | Crédit | Solde (XOF)
    Clé : calibration dynamique des frontières de colonnes via les en-têtes du PDF,
    car les positions varient selon la version du relevé (Oct-2024 vs Jan-2026).
      - SGBS TRC Jan-2026  : Débit x0≈358, Crédit x0≈437, Solde x0≈507
      - SGBS autre version : Débit x0≈430, Crédit x0≈510, Solde x0≈580
    """
    info = _afr_header(pages_text)
    txns = []
    SKIP = {'TOTAUX DES','NOUVEAU SOLDE','SOLDE PRECEDENT','PROGRAMME DE',
            ...
            'TOTAL DES OPÉRATIONS','TOTAL DES OPERATIONS','NOUVEAU SOLDE EN',
            'REPORT DU'}

    CREDIT_KW = {'VIREMENT','VERSEMENT','REMISE','CREDIT','RECU','SWIFT',
                 'DECAISSEMENT','MAINLEVEE','DEBLOCAGE','TRF-RECU','TRF RECU',
                 'REM.CHQ','REMISE CHQ','TRF-REÇU','SORT CHEQUES'}
    DEBIT_KW  = {'REDEVANCE','COMMISSION','FRAIS','ECHEANCE','PRET',
                 'FACTURATION','RETRAIT','AGIOS','PRELEVEMENT','PRLV',
                 'CHQ COMP','RETRAIT ESPECES','IMPAYE','IBE','ABONNEMENT',
                 'FRAIS TELECOMP','FRAIS VIRTREG'}

    # ── Calibration dynamique via les en-têtes de colonnes ───────────────────
    # On lit la position x0 des tokens "Débit", "Crédit", "Solde" sur la 1ère page
    # pour calculer les vraies frontières de CE relevé précis.
    # Valeurs par défaut (SGBS TRC Jan-2026 mesuré) :
    _debit_hdr_x  = 358
    _credit_hdr_x = 437
    _solde_hdr_x  = 507

    for pw in pages_words[:1]:
        for w in pw:
            wt = w['text'].strip().upper()
            if wt in ('DÉBIT', 'DEBIT') and w['x0'] > 200:
                _debit_hdr_x  = w['x0']
            elif wt in ('CRÉDIT', 'CREDIT') and w['x0'] > _debit_hdr_x:
                _credit_hdr_x = w['x0']
            elif wt in ('SOLDE', 'SOLDE(XOF)', 'SOLDE (XOF)') and w['x0'] > _credit_hdr_x:
                _solde_hdr_x  = w['x0']

    # Frontières : milieu entre chaque paire de colonnes
    DEBIT_X_START        = _debit_hdr_x  - 5     # début zone débit
    DEBIT_CREDIT_BOUNDARY = (_debit_hdr_x + _credit_hdr_x) / 2   # séparation débit/crédit
    CREDIT_X_START       = _credit_hdr_x - 5     # début zone crédit
    SOLDE_X_START        = _solde_hdr_x  - 5     # début zone solde (ignorée)
    ...
    for pw in pages_words:
        ...
        while i < len(rows):
            ...
            label = ' '.join(w['text'] for w in row
                             if 115 <= w['x0'] < DEBIT_X_START).strip()
            label_up = label.upper()
            if not label or any(s in label_up for s in SKIP):
                i += 1; continue
            if any(label_up.startswith(s) for s in ('SOLDE', 'TOTAL', 'REPORT')):
                i += 1; continue

            # Mots numériques à droite du libellé
            right_words = sorted([w for w in row if w['x0'] >= DEBIT_X_START],
                                 key=lambda w: w['x0'])
            ...
            # Éliminer le solde (blocs dont x0 ≥ SOLDE_X_START)
            candidates = [r for r in resolved if r[1] < SOLDE_X_START]
            if not candidates:
                candidates = resolved[:-1] if nb >= 2 else resolved

            if len(candidates) == 1:
                val, x0 = candidates[0]
                if x0 >= CREDIT_X_START:
                    credit_amt = val
                elif x0 < DEBIT_CREDIT_BOUNDARY:
                    debit_amt = val
                else:
                    # Zone ambiguë entre les deux colonnes → sémantique
                    if is_credit_kw and not is_debit_kw:
                        credit_amt = val
                    elif is_debit_kw and not is_credit_kw:
                        debit_amt = val
                    else:
                        debit_amt = val  # défaut conservateur

            elif len(candidates) == 2:
                val_l, x0_l = min(candidates, key=lambda x: x[1])
                val_r, x0_r = max(candidates, key=lambda x: x[1])
                if x0_l < DEBIT_CREDIT_BOUNDARY and x0_r >= CREDIT_X_START:
                    debit_amt  = val_l
                    credit_amt = val_r
                elif x0_l >= CREDIT_X_START:
                    # Les deux en zone crédit (montant fragmenté en 2 tokens)
                    credit_amt = val_l
                elif x0_r < DEBIT_CREDIT_BOUNDARY:
                    # Les deux en zone débit (montant fragmenté en 2 tokens)
                    debit_amt = val_l
                else:
                    if is_credit_kw and not is_debit_kw:
                        credit_amt = val_r
                ...
                # ≥ 3 candidats : le plus à gauche = débit, le plus à droite = crédit
                x_min_c = min(candidates, key=lambda x: x[1])
                x_max_c = max(candidates, key=lambda x: x[1])
                if x_min_c[1] < DEBIT_CREDIT_BOUNDARY and x_max_c[1] >= CREDIT_X_START:
                    debit_amt  = x_min_c[0]
                    credit_amt = x_max_c[0]
                elif x_max_c[1] >= CREDIT_X_START:
                    credit_amt = x_max_c[0]
                else:
                    if is_credit_kw and not is_debit_kw:
                        credit_amt = x_max_c[0]
...
#   Crédit     : x0 ≈ 470–560
#   Solde      : x0 ≥ 560  (ignoré)
# Particularité : montants XOF entiers fragmentés en plusieurs tokens pdfplumber
# ════════════════════════════════════════════════════════════════════════════
def parse_cbao(pages_words, pages_text, _pdf_path=''):
    ...
    SKIP = {'TOTAL', 'SOLDE', 'DATE', 'VALEUR', 'LIBELLÉ', 'LIBELLE', 'DÉBIT', 'DEBIT',
            ...
            'NOM DU CLIENT', 'NUMÉRO DE COMPTE', 'LIBELLÉ DU COMPTE', 'CODE IBAN'}
    SKIP_START = ('SOLDE', 'TOTAL', 'PAGE', '(*)')
    ...
    # Calibration dynamique des colonnes débit/crédit
    debit_x_min  = 370
    debit_x_max  = 470
    credit_x_min = 470
    credit_x_max = 570
    ...
    for pw in pages_words[:1]:
        for w in pw:
            ...
            if wt in ('DÉBIT', 'DEBIT', 'DÉBIT (XOF)', 'DEBIT (XOF)') and w['x0'] > 200:
                debit_x_min = max(200, w['x0'] - 20)
                debit_x_max = w['x0'] + 110
            elif wt in ('CRÉDIT', 'CREDIT', 'CRÉDIT (XOF)', 'CREDIT (XOF)') and w['x0'] > 300:
                credit_x_min = max(300, w['x0'] - 20)
                credit_x_max = w['x0'] + 110
    ...
    for pw in pages_words:
        ...
        while i < len(rows):
            ...
            # Libellé : x0 ≈ 150–380
            label_words = [w for w in row if 150 <= w['x0'] < 380]
            label = ' '.join(w['text'] for w in label_words).strip()
            label_up = label.upper()
            if not label or any(s in label_up for s in SKIP):
                i += 1; continue
            ...
            # Montants débit/crédit (entiers XOF fragmentés)
            debit_words  = [w for w in row if debit_x_min  <= w['x0'] < debit_x_max]
            credit_words = [w for w in row if credit_x_min <= w['x0'] < credit_x_max]
            debit_amt  = _uba_join_amount(debit_words)
            credit_amt = _uba_join_amount(credit_words)

            # Si pas de montant détecté, essai avec zones élargies
            if debit_amt is None and credit_amt is None:
                all_right = sorted([w for w in row if w['x0'] >= 300], key=lambda x: x['x0'])
                num_blocks = []
                cur = []
                prev_x1 = None
                for w in all_right:
                    is_num = bool(re.match(r'^[\d,\.]+$', w['text']) and re.search(r'\d', w['text']))
                    if not is_num:
                        if cur: num_blocks.append(cur); cur = []
                        prev_x1 = None; continue
                    if prev_x1 is not None and (w['x0'] - prev_x1) > 40:
                        if cur: num_blocks.append(cur)
                        cur = [w]
                    else:
                        cur.append(w)
                    prev_x1 = w.get('x1', w['x0'] + 20)
                if cur: num_blocks.append(cur)
                resolved = [(v, b[0]['x0']) for b in num_blocks
                            for v in [_uba_join_amount(b)] if v is not None]
                candidates = resolved[:-1] if len(resolved) >= 3 else resolved
                if len(candidates) == 1:
                    val, x0 = candidates[0]
                    mid = (debit_x_min + credit_x_max) / 2
                    if x0 >= mid:
                        credit_amt = val
                    else:
                        debit_amt = val
            ...
            while j < len(rows):
                ...
                if r2 and any(w['x0'] < 80 and re.match(r'^\d{2}/\d{2}/\d{4}$', w['text']) for w in r2):
                    break
                nl = ' '.join(w['text'] for w in r2 if 150 <= w['x0'] < 380).strip()
                if nl and not any(s in nl.upper() for s in SKIP):
                    memo_parts.append(nl)
                j += 1
            i = j
...
#   Débit       : x0 ≈ 490–580  (montants négatifs dans la colonne Débit → crédit)
#   Crédit      : x0 ≈ 580–660
#   Solde       : x0 ≥ 660 (ignoré)
# Particularité : les montants Débit sont négatifs (ex: -200 000,00) et les
#   Crédits positifs (ex: 600 200,00). La colonne "Solde courant" est à droite.
# ════════════════════════════════════════════════════════════════════════════
def parse_boa(pages_words, pages_text, _pdf_path=''):
    ...
    SKIP = {'TOTAL', 'SOLDE', 'DATE', 'VALEUR', 'DESCRIPTION', 'RÉFÉRENCE', 'REFERENCE',
            ...
            'BMCE', 'SOLDE DISPONIBLE', 'SOLDE D\'OUVERTURE', 'SOLDE DE CLÔTURE',
            'SOLDE D\'OUVERTURE', 'DÉTAILS DU COMPTE', 'DETAILS DU COMPTE'}
    SKIP_START = ('SOLDE', 'TOTAL', 'FRAIS', 'DROIT', 'TAXE', 'COMMISSION',
                  'PRELEV', 'PRÉLEV', 'BANK OF AFRICA')
    ...
    if m_close:
        ...
        if v:
            info['balance_close'] = v
    # Format alternatif BOA : "Solde de clôture 2 137 592,00 XOF"
    if not info.get('balance_close'):
        m_close2 = re.search(r'Solde\s+de\s+cl[ôo]ture\s+([\d\s]+[,\.]?\d*)', full_text, re.IGNORECASE)
        if m_close2:
            v = parse_amount(m_close2.group(1).replace(' ', ''))
            if v:
                info['balance_close'] = v

    def _boa_parse_amount_raw(words):
        """Parse un montant BOA qui peut être négatif (ex: -200 000,00)."""
        if not words:
            return None
        full = ' '.join(w['text'] for w in words).replace('\xa0', ' ').strip()
        neg = full.startswith('-')
        full = full.lstrip('-').strip()
        v = _uba_join_amount(words) or parse_amount(full)
        if v is not None and v > 0:
            return -v if neg else v
        return None

    # Calibration dynamique des colonnes sur la première page
    debit_x_min  = 490
    debit_x_max  = 580
    credit_x_min = 580
    credit_x_max = 680
    ...
    for pw in pages_words[:1]:
        for w in pw:
            wt = w['text'].upper().strip()
            if wt in ('DÉBIT', 'DEBIT') and w['x0'] > 300:
                debit_x_min = max(300, w['x0'] - 20)
                debit_x_max = w['x0'] + 110
            elif wt in ('CRÉDIT', 'CREDIT') and w['x0'] > 400:
                credit_x_min = max(400, w['x0'] - 20)
                credit_x_max = w['x0'] + 110
    ...
    for pw in pages_words:
        ...
        while i < len(rows):
            ...
            # Montants débit/crédit avec zones calibrées
            debit_words  = [w for w in row if debit_x_min  <= w['x0'] < debit_x_max]
            credit_words = [w for w in row if credit_x_min <= w['x0'] < credit_x_max]
            debit_raw  = _boa_parse_amount_raw(debit_words)
            credit_raw = _boa_parse_amount_raw(credit_words)
            ...
            # debit_raw négatif = crédit (ex: remboursement affiché en débit négatif)
            if debit_raw is not None:
                if debit_raw < 0:
                    txns.append(_make_txn(date_ofx, abs(debit_raw), name, memo))
                else:
                    txns.append(_make_txn(date_ofx, -debit_raw, name, memo))
            elif credit_raw is not None and credit_raw > 0:
                txns.append(_make_txn(date_ofx, credit_raw, name, memo))
...
#   Crédit     : x0 ≈ 520–610
#   Solde      : x0 ≥ 610  (ignoré)
# Particularité : montants XOF entiers fragmentés, libellé peut être multi-ligne
# ════════════════════════════════════════════════════════════════════════════
def parse_orabank(pages_words, pages_text, _pdf_path=''):
    ...
    SKIP = {'TOTAL', 'SOLDE', 'DATE', 'VALEUR', 'LIBELLÉ', 'LIBELLE', 'DÉBIT', 'DEBIT',
            ...
            'TOTAL GÉNÉRAL', 'TOTAL GENERAL', 'TOTAL GÉNÉRAL DES MOUVEMENTS',
            'SOLDE AU', 'VEUILLEZ NOTER'}
    SKIP_START = ('SOLDE', 'TOTAL', 'ANCIEN SOLDE', 'NOUVEAU SOLDE', 'VEUILLEZ')

    full_text = ' '.join(pages_text)
    # Solde final "Solde au : 31/12/2025  18 364 483"
    m_bal = re.search(r'Solde\s+au\s*[:\-]?\s*[\d/]+\s+([\d\s]+)', full_text, re.IGNORECASE)
    if m_bal:
        v = parse_amount(m_bal.group(1).strip().replace(' ', ''))
        if v:
            info['balance_close'] = v

    # Détecter les limites de colonnes dynamiquement sur la première page
    # en cherchant les en-têtes "Débit" et "Crédit"
    debit_x_min  = 430
    debit_x_max  = 520
    credit_x_min = 520
    credit_x_max = 620

    for pw in pages_words[:1]:  # première page seulement pour calibrer
        for w in pw:
            wt = w['text'].upper().strip()
            if wt in ('DÉBIT', 'DEBIT') and w['x0'] > 300:
                debit_x_min = max(300, w['x0'] - 20)
                debit_x_max = w['x0'] + 100
            elif wt in ('CRÉDIT', 'CREDIT') and w['x0'] > 350:
                credit_x_min = max(350, w['x0'] - 20)
                credit_x_max = w['x0'] + 100
    ...
    for pw in pages_words:
        ...
        while i < len(rows):
            row = rows[i]
            # Date : x0 < 75, DD/MM/YYYY
            date_words = [w for w in row if w['x0'] < 75 and re.match(r'^\d{2}/\d{2}/\d{4}$', w['text'])]
            if not date_words:
                i += 1; continue
            date_str = date_words[0]['text']

            # Libellé : x0 ≈ 75–370
            label_words = [w for w in row if 75 <= w['x0'] < 370]
            label = ' '.join(w['text'] for w in label_words).strip()
            label_up = label.upper()
            if not label or len(label) < 3:
                i += 1; continue
            ...
            # Montants : chercher dans les zones débit et crédit calibrées
            debit_words  = [w for w in row if debit_x_min  <= w['x0'] < debit_x_max]
            credit_words = [w for w in row if credit_x_min <= w['x0'] < credit_x_max]
            debit_amt  = _uba_join_amount(debit_words)
            credit_amt = _uba_join_amount(credit_words)

            # Si on ne trouve rien avec les zones calibrées, essai zones élargies
            if debit_amt is None and credit_amt is None:
                all_right = [w for w in row if w['x0'] >= 370]
                # Grouper les blocs numériques à droite
                num_blocks = []
                cur_block = []
                prev_x1 = None
                for w in sorted(all_right, key=lambda x: x['x0']):
                    is_num = bool(re.match(r'^[\d\s,\.]+$', w['text']) and re.search(r'\d', w['text']))
                    if not is_num:
                        if cur_block: num_blocks.append(cur_block); cur_block = []
                        prev_x1 = None; continue
                    if prev_x1 is not None and (w['x0'] - prev_x1) > 40:
                        if cur_block: num_blocks.append(cur_block)
                        cur_block = [w]
                    else:
                        cur_block.append(w)
                    prev_x1 = w.get('x1', w['x0'] + 20)
                ...
                if resolved:
                    # Filtrer le solde (dernier bloc si ≥ 3 blocs)
                    candidates = resolved[:-1] if len(resolved) >= 3 else resolved
                    if len(candidates) == 1:
                        val, x0 = candidates[0]
                        # Approximation : si x0 dans la moitié droite → crédit
                        mid = (debit_x_min + credit_x_max) / 2
                        if x0 >= mid:
                            credit_amt = val
                        else:
            ...
            while j < len(rows):
                r2 = rows[j]
                if r2 and any(w['x0'] < 75 and re.match(r'^\d{2}/\d{2}/\d{4}$', w['text'])
                               for w in r2):
                    break
                nl = ' '.join(w['text'] for w in r2 if 75 <= w['x0'] < 370).strip()
                if nl and not any(s in nl.upper() for s in SKIP):
                    memo_parts.append(nl)
                j += 1
            i = j
...
#   Détails       : x0 ≈ 80–420 (description longue)
#   Cheque N°     : peut être dans 200–300
#   Agence        : x0 ≈ 300–370 (ignoré)
#   Date Valeur   : x0 ≈ 370–430 (ignoré)
#   Mouv. Débit   : x0 ≈ 430–520
#   Mouv. Crédit  : x0 ≈ 520–610
#   Solde         : x0 ≥ 610  (ignoré)
# Particularité IROKO BEACH : les virements TPE (SV CLEARING) génèrent 3 lignes
#   par transaction (PRIN=montant principal, COMM=commission, TAX=taxe).
#   On consolide : seule la ligne PRIN est importée comme transaction,
#   COMM et TAX sont ignorées (frais).
# ════════════════════════════════════════════════════════════════════════════
def parse_nsia(pages_words, pages_text, _pdf_path=''):
    ...
    # Libellés NSIA à ignorer car ce sont des frais/commissions sur opérations TPE
    # (COMM = commission, TAX = taxe sur intérêts) — on ne garde que PRIN
    SKIP_TPE_SUFFIX = (':PURCHASE', 'COMM', ' TAX', ':COMM', ':TAX')
    ...
    # Détection dynamique des zones de colonnes (calibration sur en-têtes)
    # Valeurs par défaut calibrées sur relevés NSIA AAG SENEGAL (mars 2026) :
    #   Débit  : x0 ≈ 382–430  (en-tête "Mouv." à 382, "Débit" à 406)
    #   Crédit : x0 ≈ 451–500  (en-tête "Mouv." à 451, "Crédit" à 475)
    #   Solde  : x0 ≈ 520+     (exclu)
    debit_x_min  = 380
    debit_x_max  = 450
    credit_x_min = 450
    credit_x_max = 525
    solde_x_min  = 520  # frontier : tout ce qui est >= solde_x_min est ignoré

    for pw in pages_words[:1]:
        for w in pw:
            wt = w['text'].upper().strip()
            if wt in ('DÉBIT', 'DEBIT', 'MOUV. DÉBIT', 'MOUV. DEBIT') and w['x0'] > 300:
                debit_x_min = max(300, w['x0'] - 20)
                debit_x_max = w['x0'] + 70   # fenêtre étroite — évite le Solde
            elif wt in ('CRÉDIT', 'CREDIT', 'MOUV. CRÉDIT', 'MOUV. CREDIT') and w['x0'] > 400:
                credit_x_min = max(400, w['x0'] - 20)
                credit_x_max = w['x0'] + 70  # fenêtre étroite — évite le Solde
            elif wt in ('SOLDE',) and w['x0'] > 450:
                solde_x_min = w['x0'] - 20   # marge généreuse pour attraper les premiers tokens
    ...
    for pw in pages_words:
        ...
        while i < len(rows):
            ...
            # Détails Transaction : x0 ≈ 80–420
            # Exclure les tokens de date valeur et les numéros d'agence purs
            label_words = [w for w in row if 80 <= w['x0'] < 420
                           and not re.match(r'^\d{2}/\d{2}/\d{4}$', w['text'])
            ...
            # Ignorer les lignes COMM et TAX des virements SV CLEARING
            # (sous-composantes de frais sur les paiements TPE)
            is_tpe_sub = (
                'SV CLEARING' in label_up and
                any(label_up.endswith(sfx) or sfx in label_up for sfx in (':COMM', ':TAX', 'COMM', ' TAX'))
            )
            # Aussi ignorer si le libellé se termine par COMM ou TAX (tokens pdfplumber)
            if is_tpe_sub:
                i += 1; continue

            # Mouv. Débit et Mouv. Crédit (exclu Solde >= solde_x_min)
            debit_words  = [w for w in row if debit_x_min  <= w['x0'] < min(debit_x_max,  solde_x_min)
                            and not re.match(r'^\d{2}/\d{2}/\d{4}$', w['text'])]
            credit_words = [w for w in row if credit_x_min <= w['x0'] < min(credit_x_max, solde_x_min)
                            and not re.match(r'^\d{2}/\d{2}/\d{4}$', w['text'])]
            debit_amt  = _uba_join_amount(debit_words)
            credit_amt = _uba_join_amount(credit_words)

            # Si pas de montant avec zones calibrées, essai élargi (hors Solde)
            if debit_amt is None and credit_amt is None:
                all_right = sorted([w for w in row if w['x0'] >= 370 and w['x0'] < solde_x_min], key=lambda x: x['x0'])
                num_blocks = []
                cur = []
                prev_x1 = None
                for w in all_right:
            ...
            while j < len(rows):
                ...
                if r2 and any(w['x0'] < 80 and re.match(r'^\d{2}/\d{2}/\d{4}$', w['text'])
                               for w in r2):
                    break
                nl = ' '.join(w['text'] for w in r2 if 80 <= w['x0'] < 420
                              and not re.match(r'^\d{2}/\d{2}/\d{4}$', w['text'])).strip()
                if nl and not any(s in nl.upper() for s in SKIP):
                    memo_parts.append(nl)
                j += 1
            i = j
...
def _eco_parse_xof_amount(text):
    """
    Parse les montants Ecobank ESN au format XOFx,xxx.xx (point décimal anglais,
    préfixe XOF) ou les montants fragmentés sans décimales (XOF entiers).
    Exemples : 'XOF6,500.00', 'XOF85,700.00', 'XOF1,714.00', '30600'
    """
    if not text:
        return None
    s = str(text).strip()
    # Format XOFx,xxx.xx ou XOFx.xx
    m = re.match(r'^XOF([\d,]+\.?\d*)$', s, re.IGNORECASE)
    if m:
        val_str = m.group(1).replace(',', '')  # supprimer séparateurs milliers
        try:
            return float(val_str)
        except ValueError:
            pass
    # Format entier ou décimal standard
    v = parse_amount(s.replace('XOF', '').strip())
...
def parse_ecobank(pages_words, pages_text, _pdf_path=''):
    ...
    - Format ESN (Agence Yoff) : date DD-Mon-YYYY, montants XOFx,xxx.xx,
      colonnes Transaction Date | Description | Reference | Value Date | Payments | Deposits | Balance
    - Format classique : Date(DD/MM/YY) | Transaction | DateVal | Débit | Crédit | Solde
    """
    info = _afr_header(pages_text)
    full_text = ' '.join(pages_text)
    ...
    if not info.get('iban') and not info.get('_rib_account'):
        # Format ESN : "Account Number 101635769001"
        m_cpte = re.search(
            r'Account\s+Number\s+([\d]{6,20})',
            full_text, re.IGNORECASE
        )
        if not m_cpte:
            m_cpte = re.search(
    ...
    if not info.get('period_start'):
        # Format ESN : "Statement From Date 01-05-2025  Statement To Date 31-05-2025"
        m_per_esn = re.search(
        ...
        if m_per_esn:
            ...
            info['period_start'] = _esn_date_fmt(m_per_esn.group(1))
            info['period_end']   = _esn_date_fmt(m_per_esn.group(2))
        else:
            m_per = re.search(
                r'(?:Du|du)\s+(\d{2}/\d{2}/\d{4})\s+(?:Au|au)\s+(\d{2}/\d{2}/\d{4})',
                full_text
    ...
    # ── Solde de clôture ─────────────────────────────────────────────────────
    # Format ESN : "Closing Balance XOF6,095,325.00"
    m_bal_esn = re.search(r'Closing\s+Balance\s+XOF([\d,]+\.?\d*)', full_text, re.IGNORECASE)
    if m_bal_esn:
        try:
            info['balance_close'] = float(m_bal_esn.group(1).replace(',', ''))
        except ValueError:
            pass
    else:
    ...
    # ── Détection format ESN (montants XOF avec point décimal anglais) ───────
    is_esn_format = bool(re.search(r'XOF[\d,]+\.\d{2}', full_text))

    SKIP_UP = {'TOTAL','SOLDE','DATE','DATEVAL','TRANSACTION',
               'DÉBIT','DEBIT','CRÉDIT','CREDIT','PERIODE','PÉRIODE',
               'ACCOUNT STATEMENT','ACCOUNT NUMBER','ACCOUNT TYPE',
               'TRANSACTION DATE','DESCRIPTION','REFERENCE NUMBER',
               'VALUE DATE','PAYMENTS','DEPOSITS','BALANCE',
               'OPENING BALANCE','CLOSING BALANCE','TOTAL DEBIT','TOTAL CREDIT'}

    # Mois en anglais pour format ESN
    _MOIS_EN = {
        'jan':'01','feb':'02','mar':'03','apr':'04','may':'05','jun':'06',
        'jul':'07','aug':'08','sep':'09','oct':'10','nov':'11','dec':'12'
    }

    def _eco_date_esn(row):
        """Date format DD-Mon-YYYY (ex: 30-May-2025) en x0 < 80."""
        for w in row:
            if w['x0'] < 80:
                m = re.match(r'^(\d{2})-([A-Za-z]{3})-(\d{4})$', w['text'])
                if m:
                    mm = _MOIS_EN.get(m.group(2).lower())
                    if mm:
                        return f"{m.group(3)}{mm}{m.group(1)}"  # YYYYMMDD directement
        return ''

    def _eco_date_classic(row):
        for w in row:
            if w['x0'] < 65 and re.match(r'^\d{2}/\d{2}/(\d{2}|\d{4})$', w['text']):
                return w['text']
        return ''

    def _eco_date_ofx_classic(ds):
        p = ds.split('/')
        if len(p) == 3:
            dd,mm,yy = p
    ...
    if is_esn_format:
        # ── Format ESN : Transaction Date | Description (multi-ligne) | Ref | Value Date | Payments | Deposits | Balance
        # Positions mesurées sur PDF Ecobank ESN MEGAMAX (mai 2025) :
        #   Transaction Date : x0 < 100 (DD-Mon-YYYY)
        #   Description      : x0 ≈ 100–360 (peut être multi-ligne : RRN + card + type)
        #   Reference Number : x0 ≈ 360–430
        #   Value Date       : x0 ≈ 430–500
        #   Payments (débit) : x0 ≈ 500–600
        #   Deposits (crédit): x0 ≈ 600–700
        #   Balance          : x0 ≥ 700
        ...
        for pw in pages_words:
            ...
            while i < len(rows):
                row = rows[i]
                date_ofx = _eco_date_esn(row)
                if not date_ofx:
                    i += 1; continue

                # Description : x0 ≈ 100–360
                label_words = [w for w in row if 100 <= w['x0'] < 360]
                label = ' '.join(w['text'] for w in label_words).strip()
                label_up = label.upper()
                if not label or any(s in label_up for s in SKIP_UP):
                    i += 1; continue
                if re.match(r'^[\d\s/\-\.]+$', label):
                    i += 1; continue

                # Récupérer les lignes de continuation (sans nouvelle date)
                memo_parts = []
                j = i + 1
                while j < len(rows):
                    r2 = rows[j]
                    if _eco_date_esn(r2):
                        break
                    cont = ' '.join(w['text'] for w in r2 if 100 <= w['x0'] < 360).strip()
                    if cont and not any(s in cont.upper() for s in SKIP_UP):
                        memo_parts.append(cont)
                    j += 1
                i = j

                # Montants sur la même ligne que la date
                # Payments (débit) : x0 ≈ 500–620
                # Deposits (crédit): x0 ≈ 620–720
                pay_words = [w for w in row if 480 <= w['x0'] < 640]
                dep_words = [w for w in row if 640 <= w['x0'] < 780]

                pay_text = ' '.join(w['text'] for w in pay_words).strip()
                dep_text = ' '.join(w['text'] for w in dep_words).strip()

                debit_amt  = _eco_parse_xof_amount(pay_text) if pay_text else None
                credit_amt = _eco_parse_xof_amount(dep_text) if dep_text else None
        ...
        # Si pas de transactions avec le parsing par mots, essayer le texte brut ESN
        if not txns:
            for page_text in pages_text:
                for line in page_text.splitlines():
                    line = line.strip()
                    # Ligne commençant par DD-Mon-YYYY
                    m_dl = re.match(r'^(\d{2}-[A-Za-z]{3}-\d{4})\s+(.+)', line)
                    if not m_dl:
                        continue
                    date_raw = m_dl.group(1)
                    rest = m_dl.group(2)
                    dd, mon_s, yyyy = date_raw.split('-')
                    mm = _MOIS_EN.get(mon_s.lower())
                    if not mm:
                        continue
                    date_ofx = f"{yyyy}{mm}{dd}"

                    rest_up = rest.upper()
                    if any(s in rest_up for s in SKIP_UP):
                        continue

                    # Chercher tous les montants XOF sur la ligne
                    amounts = re.findall(r'XOF([\d,]+\.?\d*)', rest, re.IGNORECASE)
                    if not amounts:
                        continue

                    # Libellé = tout le texte avant les montants XOF
                    label_part = re.sub(r'\s+XOF[\d,.]+.*$', '', rest, flags=re.IGNORECASE).strip()
                    if not label_part or len(label_part) < 3:
                        continue

                    # Dernier montant = balance, avant-dernier = opération
                    if len(amounts) >= 2:
                        amt_str = amounts[-2]
                        bal_str = amounts[-1]
                        try:
                            amt = float(amt_str.replace(',', ''))
                            # On ne peut déterminer le sens sans solde précédent ici
                            # Utiliser le contexte du libellé
                            is_credit = any(k in label_part.upper() for k in
                                          ('PRIN', 'DEPOSIT', 'CREDIT', 'REMISE', 'VERSEMENT',
                                           'B/O', 'RETOUR'))
                            if amt > 0:
                                signed = amt if is_credit else -amt
                                txns.append(_make_txn(date_ofx, signed, label_part))
                        except ValueError:
                            pass
                    elif len(amounts) == 1:
                        try:
                            ...
                            if amt > 0:
                                is_credit = any(k in label_part.upper() for k in
                                              ('PRIN', 'DEPOSIT', 'CREDIT', 'B/O', 'RETOUR'))
                                signed = amt if is_credit else -amt
                                txns.append(_make_txn(date_ofx, signed, label_part))
                        except ValueError:
                            pass
        ...
        # ── Format classique Ecobank (DD/MM/YY) ──────────────────────────────
        for pw in pages_words:
            rows = group_words_by_row(pw, tol=4.0)
            i = 0
            while i < len(rows):
                row = rows[i]
                date_str = _eco_date_classic(row)
                if not date_str:
                    i += 1; continue
                ...
                date_ofx = _eco_date_ofx_classic(date_str)
                name, memo = smart_label(label, memo_parts)
                if debit_amt and debit_amt > 0:
                    txns.append(_make_txn(date_ofx, -debit_amt, name, memo))
                elif credit_amt and credit_amt > 0:
