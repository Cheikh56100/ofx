...
def parse_nsia(pages_words, pages_text, _pdf_path=''):
    ...
    for pw in pages_words:
        ...
        while i < len(rows):
            ...
            # Le terminal TPE NSIA génère 3 lignes par transaction :
            #   1. :PURCHASE  OPID=xxxx COMM → frais réseau (petit montant) → IGNORER
            #   2. :PURCHASE  OPID=xxxx TAX  → TVA frais réseau → IGNORER
            #   3. :-PURCHASE OPID=xxxx      → montant principal → GARDER (crédit)
            # Et pour débits TPE :
            #   1. :PURCHASE OPID=xxxx COMM  → frais
            #   2. :PURCHASE OPID=xxxx TAX   → TVA
            #   3. :PURCHASE OPID=xxxx PRIN  → montant principal → GARDER (débit)
            # On détecte les lignes de frais/TVA via leur libellé court et montant < 5000
            is_tpe_frais = False
            if 'SV CLEARING' in label_up or 'CLEARING' in label_up:
                # Ligne de frais/TVA : montant < 5000 XOF ET pas de tiret dans :-PURCHASE
                if debit_amt and debit_amt < 5000 and ':-PURCHASE' not in label_up:
                    is_tpe_frais = True
                elif credit_amt and credit_amt < 5000 and ':-PURCHASE' not in label_up:
                    is_tpe_frais = True

            if is_tpe_frais:
                pass  # Ignorer les lignes de frais/commissions TPE
            elif debit_amt and debit_amt > 0:
                txns.append(_make_txn(date_ofx, -debit_amt, name, memo))
            elif credit_amt and credit_amt > 0:
                txns.append(_make_txn(date_ofx, credit_amt, name, memo))
    ...
    if not any(t for t in txns if t and 'CLEARING' in t.get('name','').upper()):
        full_text_nsia = '\n'.join(pages_text)
        ...
        for idx, line in enumerate(lines_nsia):
            ...
            is_credit_raw = (':-PURCHASE' in label_up_raw or
                             '- PURCHASE' in label_up_raw or
                             any(k in label_up_raw for k in CREDIT_KW))
            is_debit_raw  = (':PURCHASE' in label_up_raw and
                             ':-PURCHASE' not in label_up_raw and
                             '-PURCHASE' not in label_up_raw)
            if not is_credit_raw and not is_debit_raw:
                is_debit_raw = any(k in label_up_raw for k in DEBIT_KW)
